...
def test_entries_list_is_empty(event: EventType) -> None:
    html = Html(
        text=render.entries_table(
            event=event, view="Entries", view_entries_list=[], columns=set()
        )
    )
...
def test_not_competing_is_true(
    ...
    elem = html.find(path=f".//table[@id='{TABLE_ID}']/tbody/tr[2]/td[1]")
    assert elem.text == "NC"
...
def test_first_name_is_defined(
    ...
    ranked_entries[0].entry.first_name = "Sabine"
    html = Html(
        text=render.entries_table(
            event=event,
            view="Entries",
...
def test_club_is_defined(
    ...
    ranked_entries[0].entry.club_name = "OC Bundestag"
    html = Html(
        text=render.entries_table(
            event=event,
            view="Entries",
...
def test_class_is_defined(
    ...
    ranked_entries[0].entry.class_id = 7
    ranked_entries[0].entry.class_name = "Elite Men"
    html = Html(
        text=render.entries_table(
            event=event,
            view="Entries",
