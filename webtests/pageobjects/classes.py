...
class AddClassDialog:
    ...
    def check_values(
        ...
        assert (
            short_name == TextControl(driver=self.driver, id="cla_shortName").get_text()
        )
        assert (
            course
            == ComboboxControl(driver=self.driver, id="cla_courseId").selected_text()
        )
        assert (
            voided_legs
            == TextControl(driver=self.driver, id="cla_voidedLegs").get_text()
        )
        assert (
            type == ComboboxControl(driver=self.driver, id="cla_type").selected_text()
        )
        assert (
            use_start_control
            == ComboboxControl(
                driver=self.driver, id="cla_startControl"
            ).selected_text()
        )
        assert (
            apply_handicap
            == CheckboxControl(driver=self.driver, id="cla_handicap").is_checked()
        )
        assert (
            mass_start == TextControl(driver=self.driver, id="cla_massStart").get_text()
        )
        assert (
            time_limit == TextControl(driver=self.driver, id="cla_timeLimit").get_text()
        )
        assert (
            penalty_controls
            == NumberControl(driver=self.driver, id="cla_penaltyControls").get_text()
        )
        assert (
            penalty_time_limit
            == NumberControl(driver=self.driver, id="cla_penaltyOvertime").get_text()
        )

    def get_course_list(self) -> list[str]:
        return ComboboxControl(driver=self.driver, id="cla_courseId").values()

    def get_type_list(self) -> list[str]:
        return ComboboxControl(driver=self.driver, id="cla_type").values()

    def get_use_start_control_list(self) -> list[str]:
        return ComboboxControl(driver=self.driver, id="cla_startControl").values()
    ...
    def enter_values(
        ...
        if name is not None:
            TextControl(driver=self.driver, id="cla_name").set_text(text=name)
        if short_name is not None:
            TextControl(driver=self.driver, id="cla_shortName").set_text(
                text=short_name
            )
        if course is not None:
            ComboboxControl(driver=self.driver, id="cla_courseId").select_by_text(
                text=course
            )
        if voided_legs is not None:
            TextControl(driver=self.driver, id="cla_voidedLegs").set_text(
                text=voided_legs
            )
        if type is not None:
            ComboboxControl(driver=self.driver, id="cla_type").select_by_text(text=type)
        if use_start_control is not None:
            ComboboxControl(driver=self.driver, id="cla_startControl").select_by_text(
                text=use_start_control
            )
        if apply_handicap is not None:
            CheckboxControl(driver=self.driver, id="cla_handicap").set_state(
                checked=apply_handicap
            )
        if mass_start is not None:
            TextControl(driver=self.driver, id="cla_massStart").set_text(
                text=mass_start
            )
        if time_limit is not None:
            TextControl(driver=self.driver, id="cla_timeLimit").set_text(
                text=time_limit
            )
        if penalty_controls is not None:
            NumberControl(driver=self.driver, id="cla_penaltyControls").set_text(
                text=penalty_controls
            )
        if penalty_time_limit is not None:
            NumberControl(driver=self.driver, id="cla_penaltyOvertime").set_text(
                text=penalty_time_limit
            )

    def submit(self) -> None:
        elem = self.driver.find_element(By.ID, "cls.formAdd")
...
class ExportClassDialog:
    ...
    def cancel(self) -> None:
        elem = self.driver.find_element(By.ID, "cls.export.form")
        elem.find_element(By.XPATH, "button[text()='Cancel']").click()
        WebDriverWait(self.driver, 10).until(EC.invisibility_of_element(element=elem))
...
class DeleteClassDialog:
    ...
    def ok(self) -> None:
        elem = self.driver.find_element(By.ID, "cls.formDelete")
...
class ClassPage:
    def __init__(self, driver: webdriver.Remote) -> None:
        self.driver = driver
        self.actions = ClassActions(driver=driver)
        self.table = ClassTable(driver=driver)

    def filter(self) -> TextControl:
        return TextControl(driver=self.driver, id="clas.filter")

    def get_event_name(self) -> str:
        return self.driver.find_element(By.ID, "clas.event_name").text

    def get_event_date(self) -> str:
        return self.driver.find_element(By.ID, "clas.event_date").text
...
class ClassActions(Actions):
    def __init__(self, driver: webdriver.Remote) -> None:
        super().__init__(driver=driver, id="clas.actions")
    ...
    def import_(self) -> ImportClassDialog:
        ...
        return ImportClassDialog(driver=self.driver)

    def export(self) -> ExportClassDialog:
        self.action(text="Export ...").click()
        return ExportClassDialog(driver=self.driver)
    ...
    def delete(self) -> DeleteClassDialog:
        ...
        return DeleteClassDialog(driver=self.driver)


class ClassTable(Table):
    def __init__(self, driver: webdriver.Remote) -> None:
        super().__init__(driver=driver, xpath="//table[@id='cls.table']")
