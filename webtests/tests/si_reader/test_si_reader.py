...
def test_if_reader_client_is_not_connected_then_reader_status_is_offline(
    ...
    si_reader_page = main_page.goto_si_reader(event=event)
    try:
        wait.until(lambda _: si_reader_page.get_event_name() == EVENT_NAME)
        wait.until(lambda _: si_reader_page.get_event_date() == EVENT_DATE)
        wait.until(lambda _: si_reader_page.get_reader_status() == "Offline")
    finally:
        si_reader_page.close()
...
def test_if_reader_client_is_stopped_then_reader_status_is_offline(
    ...
    try:
        ...
        wait.until(lambda _: si_reader_page.get_event_name() == EVENT_NAME)
        wait.until(lambda _: si_reader_page.get_event_date() == EVENT_DATE)
        wait.until(lambda _: si_reader_page.get_reader_status() == "Disconnected")
...
def test_if_a_si_reader_unit_is_found_then_reader_status_is_connected(
    ...
    try:
        ...
        wait.until(lambda _: si_reader_page.get_event_name() == EVENT_NAME)
        wait.until(lambda _: si_reader_page.get_event_date() == EVENT_DATE)
        wait.until(lambda _: si_reader_page.get_reader_status() == "Connected")
    finally:
        si_reader_page.close()
...
def test_if_a_si_card_is_inserted_then_reader_status_is_card_reading(
    ...
    try:
        ...
        wait.until(lambda _: si_reader_page.get_event_name() == EVENT_NAME)
        wait.until(lambda _: si_reader_page.get_event_date() == EVENT_DATE)
        wait.until(lambda _: si_reader_page.get_reader_status() == "Reading card")
    finally:
        si_reader_page.close()
...
def test_if_a_si_card_is_read_then_reader_status_is_connected(
    client: WebSocketClient, main_page: MainPage, event: str
) -> None:
    wait = main_page.wait(timeout=10)
