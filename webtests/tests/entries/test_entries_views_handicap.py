...
def result_data(main_page: MainPage, event: str) -> None:
    """Load course data and result data.

    To recompute the results after changing class parameters (e.g. handicap rule),
    we need classes with assigned courses. Therefore, we load not only a ResultList
    but also a CourseData list.
    """
    course_data_path = pathlib.Path(__file__).parent.parent / "data" / "CourseData.xml"
    result_list_path = pathlib.Path(__file__).parent.parent / "data" / "ResultList.xml"

    courses_page = main_page.goto_courses(event=event)
    dialog = courses_page.actions.import_()
    dialog.import_file(path=course_data_path)
...
@pytest.mark.usefixtures("result_data")
class TestWithoutHandicap:

    @pytest.fixture(scope="class", autouse=True)
    @classmethod
    def set_handicap(cls, main_page: MainPage, event: str) -> None:
        class_page = main_page.goto_classes(event=event)

        class_page.select_class(name="Bahn A - Frauen")
        dialog = class_page.actions.edit()
        dialog.enter_values(apply_handicap=False)
        dialog.submit()

        class_page.select_class(name="Bahn A - Männer")
        dialog = class_page.actions.edit()
        dialog.enter_values(apply_handicap=False)
        dialog.submit()

    def test_if_view_is_entries_then_group_by_entries_sorted_by_name(
        self, main_page: MainPage, event: str
    ) -> None:
        entry_page = main_page.goto_entries(event=event)
...
@pytest.mark.usefixtures("result_data")
class TestWithHandicapForOneClass:
    ...
    def set_handicap(cls, main_page: MainPage, event: str) -> None:
        ...
        class_page.select_class(name="Bahn A - Männer")
        dialog = class_page.actions.edit()
        dialog.enter_values(apply_handicap=True)
        dialog.submit()
...
@pytest.mark.usefixtures("result_data")
class TestWithHandicapForAllClasses:
